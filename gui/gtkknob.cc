...
#define GTK_QUACK_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_QUACK_KNOB, GtkQuackKnobPrivate))

G_DEFINE_TYPE(GtkQuackKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkQuackKnobPrivate GtkQuackKnobPrivate;

struct _GtkQuackKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkQuackKnobPrivate *priv = GTK_QUACK_KNOB_GET_PRIVATE(widget);
...
{
    GtkQuackKnob *knob = GTK_QUACK_KNOB(object);
...
/****************************************************************
 ** init the GtkQuackKnobClass
 */

static void gtk_knob_class_init (GtkQuackKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkQuackKnobPrivate));
}
...
static void gtk_knob_init (GtkQuackKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_QUACK_KNOB, NULL ));
