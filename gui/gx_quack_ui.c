...
static void ref_value_changed(GtkAdjustment *adj, gpointer* args) {
    ...
    gx_quackUI* ui = (gx_quackUI*)arg->ui;
    if (ui->block) {
        ui->block = 0;
        return;
    }
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor*   descriptor,
    ...
    *widget = NULL;
    ...
    set_expose_func(GX_QUACK_PAINT_BOX(ui->pbox),"pedal_expose");
    gtk_container_set_border_width(GTK_CONTAINER(ui->pbox),20);
    ui->box = gtk_vbox_new(FALSE, 4);
    ui->hbox = gtk_vbox_new(TRUE, 4);
    gtk_container_set_border_width(GTK_CONTAINER(ui->hbox),10);
    ...
    gtk_box_pack_start(GTK_BOX(ui->box), ui->vbox, TRUE, TRUE, 40);
    ...
    set_expose_func(GX_QUACK_PAINT_BOX(ui->pf1box),"frame_expose");
    ...
    set_expose_func(GX_QUACK_PAINT_BOX(ui->pf2box),"frame_expose");
    ...
    ui->adj[2] = gtk_adjustment_new( 0.0, -12.0, 6.0, 0.01, 0.01*10.0, 0);
    ui->knob[2] = gtk_knob_new_with_adjustment(GTK_ADJUSTMENT(ui->adj[2]));
    ui->label[2] = gtk_label_new("GAIN");
    ...
    ui->adj[6] = gtk_adjustment_new( 0.0, 0.0, 2.0, 1.0, 1.0*10.0, 0);
    ui->knob[6] = gtk_select2_new_with_adjustment(GTK_ADJUSTMENT(ui->adj[6]));
    ui->label[6] = gtk_label_new("TONE");
    ...
    gtk_box_pack_start(GTK_BOX(ui->pf1box), ui->vkbox[6], TRUE, TRUE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(ui->pf2box), ui->vkbox[0], TRUE, TRUE, 0);
    ...
    ui->args[0]->ui = ui;
    ui->args[0]->port_index = (int)DEPTH;
    g_signal_connect(G_OBJECT(ui->adj[0]), "value-changed",
          G_CALLBACK(ref_value_changed),(gpointer*)ui->args[0]);

    ui->adj[5] = gtk_adjustment_new( 0.0, 0.0, 1.0, 1.0, 1.0*10.0, 0);
    ui->knob[5] = gtk_select3_new_with_adjustment(GTK_ADJUSTMENT(ui->adj[5]));
    ui->label[5] = gtk_label_new("RANGE");
    ...
    ui->args[5]->ui = ui;
    ui->args[5]->port_index = (int)RANGE;
    g_signal_connect(G_OBJECT(ui->adj[5]), "value-changed",
          G_CALLBACK(ref_value_changed),(gpointer*)ui->args[5]);

    ui->adj[3] = gtk_adjustment_new( 0.0, 0.0, 2.0, 1.0, 1.0*10.0, 0);
    ui->knob[3] = gtk_select_new_with_adjustment(GTK_ADJUSTMENT(ui->adj[3]));
    ui->label[3] = gtk_label_new("MODE");
    ...
    gtk_box_pack_start(GTK_BOX(ui->pf2box), ui->vkbox[3], TRUE, TRUE, 0);
