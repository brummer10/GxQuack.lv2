...
static void resize_event(gx_quackUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_quackUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_quackUI* ui = (gx_quackUI*)malloc(sizeof(gx_quackUI));
	...
	ui->controls[1] = (gx_controller) {{ 0.0, 0.0, -12.0, 6.0, 0.1},{ 120, 60, 61, 61}, false,"GAIN", KNOB, GAIN};
	ui->controls[2] = (gx_controller) {{ 0.0, 0.0, 0.0, 1.0, 1.0},{ 190, 60, 61, 61}, false,"DRIVE", SWITCH, DRIVE};
	ui->controls[3] = (gx_controller) {{ 0.0, 0.0, 0.0, 2.0, 1.0},{ 260, 60, 61, 61}, false,"TONE", ENUM, TONE};

	ui->controls[4] = (gx_controller) {{ 5.0, 5.0, -6.0, 20.0, 0.2},{ 50, 160, 61, 61}, false,"PEAK", KNOB, PEAK};
	ui->controls[5] = (gx_controller) {{ 1.0, 1.0, 0.0, 1.0, 0.01},{ 120, 160, 61, 61}, false,"DEPTH", KNOB, DEPTH};
	ui->controls[6] = (gx_controller) {{ 0.0, 0.0, 0.0, 1.0, 1.0},{ 190, 160, 61, 61}, false,"RANGE", SWITCH, RANGE};
	ui->controls[7] = (gx_controller) {{ 0.0, 0.0, 0.0, 2.0, 1.0},{ 260, 160, 61, 61}, false,"MODE", ENUM, MODE};
	ui->block_event = -1;
	...
	ui->width = ui->init_width -140 + (70 * CONTROLS/2);
	...
	*widget = (void*)ui->win;
	...
	ui->rescale.xc = (double)ui->width/(ui->init_width-140 + (70 * CONTROLS/2));
...
static void cleanup(LV2UI_Handle handle) {
	gx_quackUI* ui = (gx_quackUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_quackUI *ui,gx_controller* knob) {
	...
	if (knob->type == KNOB && knob->is_active == true) {
		...
		if (knob->port == DRIVE) {
		...
		} else if (knob->port == MODE) {
			cairo_text_extents(ui->crf,"Low", &extents);
			cairo_move_to (ui->crf, knobx1-knob_x/2.4-extents.width/1.4, knoby1+knob_y/2+extents.height);
			cairo_show_text(ui->crf, "Low");

			cairo_text_extents(ui->crf,"Mid", &extents);
			cairo_move_to (ui->crf, knobx1-extents.width/2, knoby1-knob_y/2-extents.height/2);
			cairo_show_text(ui->crf, "Mid");

			cairo_text_extents(ui->crf,"Orig", &extents);
			cairo_move_to (ui->crf, knobx1+knob_x/2.6-extents.width/2.3, knoby1+knob_y/2+extents.height/1.3);
			cairo_show_text(ui->crf, "Orig");
		} else if (knob->port == TONE) {
			cairo_text_extents(ui->crf,"Off", &extents);
			cairo_move_to (ui->crf, knobx1-knob_x/2.4-extents.width/1.4, knoby1+knob_y/2+extents.height);
			cairo_show_text(ui->crf, "Off");
			...
			cairo_move_to (ui->crf, knobx1+knob_x/2.6-extents.width/3.0, knoby1+knob_y/2+extents.height/1.3);
			cairo_show_text(ui->crf, "High");
		} else if (knob->port == RANGE) {
			cairo_text_extents(ui->crf,"Low", &extents);
			cairo_move_to (ui->crf, knobx1-knob_x/2.6-extents.width/1.4, knoby1+knob_y/2+extents.height);
			cairo_show_text(ui->crf, "Low");
...
static void bypass_expose(gx_quackUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_quackUI *ui, gx_controller* controller) {
...
static void _expose(gx_quackUI *ui) {
	static const char* plug_name = "GxQuack" ;
	...
	cairo_scale (ui->cr, 1.92, 2.5);
	cairo_set_source_surface (ui->cr, ui->frame1, 10, 18);
	cairo_paint (ui->cr);
	cairo_set_source_surface (ui->cr, ui->frame1, 10, 60);
	cairo_paint (ui->cr);
	cairo_scale (ui->cr, 1/1.92, 1/2.5);
...
static void controller_expose(gx_quackUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);

	//cairo_set_source_surface (ui->cr, ui->pedal, 0, 0);
	cairo_scale (ui->cr, 1.92, 5.5);
	cairo_set_source_surface (ui->cr, ui->frame1, 10, 8);
	
	cairo_scale (ui->cr, 1/1.92, 1/5.5);
...
static void send_controller_event(gx_quackUI *ui, int controller) {
...
static void check_value_changed(gx_quackUI *ui, int i, float* value) {
...
static void check_is_active(gx_quackUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_quackUI *ui) {
...
static bool get_active_ctl_num(gx_quackUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_quackUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_quackUI *ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_quackUI *ui, int i) {
	float value;
...
static void motion_event(gx_quackUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_quackUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_quackUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_quackUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_quackUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_quackUI *ui, bool set) {
...
static void event_handler(gx_quackUI *ui) {
	XEvent xev;
